...
- **Scenario expansion complete (2026-05-04):** All 40 scenarios written (S-01–S-40). Remaining 30 scenarios added to `tests/scenarios/golden/`. Two new rubric stubs: R-10 (requirements-tracer BDD negotiation) and R-28 (code-implementer security path). `tool_backed.py` updated to support `artifact_lang`/`artifact_ext` fields (enables SQL artifact extraction for S-22).
